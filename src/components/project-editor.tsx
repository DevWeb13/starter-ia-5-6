...
export function ProjectEditor({ id }: { id: string }) {
  const initial = () => { try { return { project: readProjects().find((item) => item.id === id) ?? null, error: null as string | null }; } catch (cause) { return { project: null, error: cause instanceof Error ? cause.message : "Impossible de lire ce projet." }; } };
  const [project, setProject] = useState<Project | null>(() => initial().project); const [loadError, setLoadError] = useState<string | null>(() => initial().error); const [saveState, setSaveState] = useState<SaveState>("saved"); const [conflict, setConflict] = useState(false); const current = useRef<Project | null>(project);
  ...
  useEffect(() => { const onStorage = (event: StorageEvent) => { if (event.key === "ai-project-launcher.projects.v1") setConflict(true); }; window.addEventListener("storage", onStorage); return () => window.removeEventListener("storage", onStorage); }, []);
